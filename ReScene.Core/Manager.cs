...
public partial class Manager
{
        ...
        int notContentAttributeIteration)
    {
        ...
        for (int j = 0; j < options.RAROptions.CommandLineArguments.Count; j++)
        {
            ...
            FireBruteForceProgress(new(options.ReleaseDirectoryPath, rarVersionDirectoryPath, joinedArguments, totalProgressSize, currentProgress, bruteForceStartDateTime) { PhaseDescription = "Phase 2: Full RAR Creation" });
            ...
            try
            {
                ...
                _logger.Information(this, $"  Params:  {joinedArguments}", LogTarget.System);
    ...
    private bool ValidateInputFiles(BruteForceOptions options)
    {
        ...
        foreach (string file in opts.ArchiveFilePaths)
        {
            ...
            if (opts.ArchiveFileCrcs.TryGetValue(file, out string? expectedCrc))
            {
                ...
                if (!string.Equals(actualCrc, expectedCrc, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    _logger.Error(this, $"  CRC MISMATCH: {file} (expected {expectedCrc}, got {actualCrc})", LogTarget.System);
                }
            }
        }
        ...
        if (missing.Count > 0 || crcMismatched.Count > 0)
        {
            ...
            _logger.Error(this, $"Input validation failed: {message}.", LogTarget.System);
            return false;
        }

        _logger.Information(this, $"Input validation passed: all {opts.ArchiveFilePaths.Count} file(s) present and verified.", LogTarget.System);
        return true;
    }
